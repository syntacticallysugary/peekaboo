...
async def receive_alert(req: SessionAlertRequest):
    ...
    classification = _CLASSIFICATION_MAP.get(req.classification, req.classification)
    ...
    await db.collection(EVENTS).document(eid).set({
        ...
        "person_id": None,
        "confidence": req.peak_similarity,
        "recording_path": req.recording_path,
        "best_frame": req.best_frame,
    })
    ...
    logger.info(
        ...
        req.camera_id, req.session_id, classification,
    )
